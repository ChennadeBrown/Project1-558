...
The following contingency table reports the number of confirmed case statuses for South Africa and Mexico from the first recorded case which were similar as well.  Based on the count these numbers appear to be based on a certain time frame.  
```{r, echo = TRUE}
tbl2 <- table(Day1$Status, Day1$Country)
tbl2
```

The following bar graph plots the output of the above contingency table which shows the confirmed case statuses for Mexico and South Africa from the first recorded case.   
```{r, echo = TRUE}
...
The following code creates a new variable which calculates the confirmed cases to recovered ratio for Greece.  The confirmed to recovered ratios were above 4:1. 
```{r, echo = TRUE, eval = FALSE, error = FALSE}
