...
The following functions returns  data for Africa & Mexico from the first recorded case. We will go through the same steps outlined above to retrieve and parse the data from the API.
...
The following contingency table reports the number of confirmed case statuses  for South Africa and Mexico from the first recorded case which were similar. 
```{r, echo = TRUE}
...
The following code calculates numerical summaries for daily cases confirmed since Day 1 for the Mexico and South Africa. These numbers are updated frequently.
```{r, echo = TRUE}
Day1 %>%
  group_by(Country) %>%
  summarize(Avg = mean(Cases), Sd = sd(Cases), Median = median(Cases), IQR =    IQR(Cases))
...
The following code returns numerical summaries for active cases in Greece.
```{r, echo = TRUE, eval = FALSE}
LiveDf %>%
